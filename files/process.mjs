...
function getProviderFiles() {
  return fs.readdirSync('.')
    .filter(f => f.startsWith('reviews-v3-') && f.endsWith('.json') && !f.includes('.prev.') && f !== 'reviews-v3-all.json')
    .sort();
}
...
function process() {
  ...
  if (!files.length) { console.error('No review files found.'); process.exit(1); }

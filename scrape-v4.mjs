...
function loadExisting(slug) {
  const path = `./reviews-v4-${slug}.json`;
...
async function scrape() {
  const inScope = PROVIDERS.filter(p => p.in_scope);
  ...
  fs.writeFileSync('reviews-v4-all.json', JSON.stringify(allReviews, null, 2));
  ...
  console.log('══════════════════════════════════════════');
  console.log(`\nDone → reviews-v4-all.json`);
  console.log(`Next step: node analyze-v4.mjs && node convert-dates.mjs && node build-dashboards-v4.mjs`);
}
